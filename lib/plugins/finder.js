...
function inject(bot) {
    ...
    function find(username, args) {
        ...
        if (blockPoints.length) {
            ...
            var distanceVec = foundBlock.pos.minus(playerEntity.position).floored().abs();
            var distance = distanceVec.x + distanceVec.y + distanceVec.z;
            bot.lookAt(blockPoints[0]);
            bot.chat('The closest ' + foundBlock.displayName + ' is at ' + foundBlock.pos + ', ' + distance + ' blocks away.');
        } else {
