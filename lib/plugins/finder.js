...
function inject(bot) {
    ...
    function find(username, args) {
        ...
        if (blockPoints.length) {
            ...
            var distanceVec = foundBlock.position.minus(playerEntity.position).floored().abs();
            var distance = distanceVec.x + distanceVec.y + distanceVec.z;
            bot.lookAt(blockPoints[0]);
            bot.chat('The closest ' + foundBlock.displayName + ' is at ' + foundBlock.position + ', ' + distance + ' blocks away.');
        } else {
