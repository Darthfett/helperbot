...
function inject(bot) {
  ...
  function increaseY() {
    ...
    var ceiling = bot.blockAt(bot.entity.position.offset(0, 2, 0));
    if (ceiling.type !== 0) {
      breakBlock(ceiling, state);
      return;
    }
    var ceiling2 = bot.blockAt(bot.entity.position.offset(0, 3, 0));
    if (ceiling2.type !== 0) {
      breakBlock(ceiling2, state);
      return;
    }
    if (! equipBuildingBlock()) return;
  ...
  function breakBlock(targetBlock, newState) {
    ...
    bot.dig(targetBlock, 99999, function(err) {
  ...
  function decreaseY() {
    if (Math.floor(bot.entity.position.y) === targetPoint.y) return changeState('start');
    console.info("decreaseY", "mine", Math.floor(bot.entity.position.y), "target", targetPoint.y);
    if (! bot.entity.onGround) return; // we're falling. do nothing.
    // dig the block below
    var targetBlock = bot.blockAt(bot.entity.position.offset(0, -1, 0));
    breakBlock(targetBlock, 'decreaseY');
  }
