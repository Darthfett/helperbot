...
function inject(bot) {
  ...
  chatCommands.registerCommand('mine.stop', stop, 0, 0);
  chatCommands.registerCommand('mine.status', tellStatus, 0, 0);
  chatCommands.registerCommand('mine.resume', resume, 0, 0);

  chatCommands.addCommandHelp('mine', "'mine block name' - start mining 'block name'. " +
      "You can add multiple target blocks.");
  chatCommands.addCommandHelp('mine.stop', "'mine stop' - stop mining.");
  chatCommands.addCommandHelp('mine.status', "'mine status' - information about the current mining job.");
  ...
  function breakBlock(targetBlock, newState) {
    ...
    if (! bot.canDigBlock(targetBlock)) {
      ...
      targetPoint.set(targetPoint.x + fuzz(), targetPoint.y + fuzz(), targetPoint.z + fuzz() );
