...
function inject(bot) {
    ...
    function getResponderFunc(chatType, username) {
        ...
        whisperFunc.username = username;
        whisperFunc.chatType = chatType;
        chatFunc.username = username;
        chatFunc.chatType = chatType;
        var typeMap = {
            'chat': chatFunc,
            'whisper': whisperFunc,
        };
    ...
    registerCommand(bot.username + ',', function(username, args, responderFunc) {
        runCommands(username, args.join(' '), responderFunc);
    });
}
...
function walkCommandTree(callback, commandRoot, leftCommand) {
    ...
    for (var key in commandRoot.commands) {
        walkCommandTree(callback, commandRoot.commands[key], leftCommand + (leftCommand.length ? ' ' : '') + key);
    }
}